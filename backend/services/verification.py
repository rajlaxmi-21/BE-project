...
def verify_deforestation_against_permits():
    ...
    WITH permit_matches AS (
        ...
        SELECT
            ...
            ST_Area(
                ST_Intersection(d.geometry, p.geometry)::geography
            ) AS intersection_area,

            ST_Area(
                ST_Intersection(d.geometry, p.geometry)::geography
            ) / NULLIF(
                ST_Area(d.geometry::geography), 0
            ) AS coverage_ratio,
    ...
    SELECT
        ...
        ROUND(
            COALESCE(intersection_area, 0)::numeric,
            2
        ) AS intersection_area_m2,

        ROUND(
            COALESCE(coverage_ratio, 0)::numeric,
            3
        ) AS coverage_ratio,

        CASE
            WHEN COALESCE(coverage_ratio, 0) >= 0.8
            THEN 'PERMITTED'
...
def calculate_summary_stats():
    ...
    ROUND(
        ...
        4
    ) AS total_green_cover_loss_km2,

    ROUND(
        COALESCE(SUM(
            CASE
                WHEN status = 'POTENTIALLY_ILLEGAL'
                THEN area_m2
                ELSE 0
            END
        ), 0) / 1000000.0,
    ...
    COUNT(
        CASE
            ...
            THEN 1
        END
    ) AS total_legal_deforestation_count
